...
class SentinelDownloader:
    def __init__(self, client_id, client_secret, base_dir = "./data/"):   
        ...
        self.oauth.register_compliance_hook("access_token_response", self.sentinelhub_compliance_hook)
...
def generate_time_ranges(start_date, end_date, frequency):
    current = start_date
    while current < end_date:
        if frequency == 'month':
            next_date = (current.replace(day=1) + timedelta(days=32)).replace(day=1)
        elif frequency == 'week':
            ...
            next_date = current + timedelta(minutes=1)
        else:
